...
export const workspaceAPI = {
  ...
  getPendingInvitations: (userId: number) =>
    api.get(`/workspaces/invitations/${userId}`).then(r => r.data),
  respondToInvitation: (inviteId: number, data: { user_id: number; action: 'accept' | 'reject' }) =>
    api.post(`/workspaces/invitations/${inviteId}/respond`, data).then(r => r.data),
  getWorkspaceInvitations: (workspaceId: number, userId: number) =>
    api.get(`/workspaces/${workspaceId}/invitations`, { params: { user_id: userId } }).then(r => r.data),
  cancelInvitation: (inviteId: number, userId: number) =>
    api.delete(`/workspaces/invitations/${inviteId}`, { params: { user_id: userId } }).then(r => r.data),
